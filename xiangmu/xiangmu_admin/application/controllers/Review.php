...
class Review extends CI_Controller {
	...
	*/
	function add(){
		...
		else
		{
			...
			$arr=$_POST;
			//进行商品添加 获取刚添加的商品id
			$goods_id=$this->Review_model->goods_add($arr);
			if($goods_id){
	...
	*/
	function page(){
    	...
    	if(is_array($data)){
			$this->load->view('review/design_fenye',$data);
		}
	}
	...
	*/
	function search(){
		...
		//查询商品的所有数据
		$sear_review=$this->Review_model->review_data($search);
		//print_r($sear_review);die;
		//判断是否有数据
		if(!isset($sear_review['0']['goods_id'])){
			echo "无查询数据";die;
		}
		$data['review']=$sear_review;
		//print_r($data);die;
		if(is_array($data)){
			$this->load->view('review/design_fenye',$data);
		}
	...
	*/
	function del(){
		...
		$re=$this->Review_model->del($review_id);
	}
	...
	*/
	...
	function up_data(){
		...
		else
		{
			...
			$arr=$_POST;
			//进行商品修改
			$re=$this->Review_model->up_data($goods_id,$arr);
			if($re){
				redirect("Goods/goods_list");
			}
		}	
